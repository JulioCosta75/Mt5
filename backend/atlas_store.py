...
Backends (selected by caller):
  * MongoDB collection ``atlas_reports`` when ``mongo_db`` is provided.
  * SQLite table ``atlas_reports`` in the same ``atlas.db`` as the MT5 cache
    when ``sqlite_path`` is provided (Windows installer / ATLAS_STORE=sqlite).
  * In-memory list otherwise (preview / offline without a store).

Every report belongs to exactly one account (``account_id``). Aggregate
cross-account snapshots are not stored here.
